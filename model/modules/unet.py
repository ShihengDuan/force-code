...
class FeatureWiseAffine(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, use_affine_level: bool = False):
        ...
        print('FeatureWiseAffine: ', in_channels, out_channels * (1+self.use_affine_level))
    def forward(self, x, time_emb):
        """Forward pass.

        Args:
            x: Input tensor of size [B, D, H, W]. B, C, T, H, W
        ...
        print(x.shape, time_emb.shape) # [12, 16, 136, 72, 144], [12, 1, 16]
        if self.use_affine_level:
            gamma, beta = self.noise_func(time_emb).view(batch_size, 1, -1, 1, 1).chunk(2, dim=1)
            # The size of gamma and beta is (batch_size, out_channels, 1, 1).
            x = (1 + gamma) * x + beta
        else:
            output = self.noise_func(time_emb).view(batch_size, -1, 1, 1, 1) # [12, 1, 16, 1, 1]
            print(output.shape, ' output from noise_func')
            x = x + output
        return x
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # print(x.shape, ' before upsample') # [12, 128, 17, 9, 18]
...
class Downsample(nn.Module):
    ...
    def __init__(self, dim: int):
        super().__init__()
        self.conv = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=3, stride=2, padding=1)
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, dim_out: int, groups: int = 32, dropout: float = 0):
        ...
        self.block = nn.Sequential(nn.GroupNorm(num_groups=groups, num_channels=dim),
                                   ...
                                   nn.Conv3d(in_channels=dim, out_channels=dim_out, kernel_size=3, padding=1))
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim: int, dim_out: int, noise_level_emb_dim: int = None, dropout: float = 0,
        ...
        self.res_conv = nn.Conv3d(in_channels=dim, out_channels=dim_out, kernel_size=1) \
    ...
    def forward(self, x, time_emb):
        ...
        Returns:
            Transformed tensor of size [B, D, H, W].
        """
        h = self.block1(x) # 5, 16, 132, 72, 144
        # print('h shape: ', h.shape, time_emb.shape)
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, in_channel: int, n_head: int = 1, norm_groups: int = 32):
        ...
        self.qkv = nn.Conv3d(in_channels=in_channel, out_channels=3*in_channel, kernel_size=1, bias=False)
        self.out = nn.Conv3d(in_channels=in_channel, out_channels=in_channel, kernel_size=1)
    ...
    def forward(self, x):
        ...
        """
        batch_size, channel, time, height, width = x.shape
        head_dim = channel // self.n_head

        norm = self.norm(x)
        qkv = self.qkv(norm).view(batch_size, self.n_head, head_dim * 3, time, height, width)
        query, key, value = qkv.chunk(3, dim=2)

        attn = torch.einsum("bncthw, bnctyx -> bnthwyx", query, key).contiguous() / math.sqrt(channel)
        attn = attn.view(batch_size, self.n_head, time, height, width, -1)
        attn = torch.softmax(attn, -1)
        attn = attn.view(batch_size, self.n_head, time, height, width, height, width)

        out = torch.einsum("bnhwyx, bncyx -> bnchw", attn, value).contiguous()
        out = self.out(out.view(batch_size, channel, time, height, width))
...
class UNet(nn.Module):
    ...
    def forward(self, x, time):
        ...
        Args:
            x: Input tensor of size: [B, C, T, H, W]
