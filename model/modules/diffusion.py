...
class GaussianDiffusion(nn.Module):
    ...
    def set_loss(self, device: torch.device):
        ...
        if self.loss_type == "l1":
            self.loss_func = nn.L1Loss().to(device)  # L1 loss.
        elif self.loss_type == "l2":
            self.loss_func = nn.MSELoss().to(device)  # Squared L2 loss.
        else:
    ...
    def p_losses(self, x_in, noise: torch.Tensor = None) -> torch.Tensor:
        ...
        """
        x, y, e = x_in
        x_start = x-e # this is the target. 
        b = x_start.shape[0]  # Dimension of s_start is (B, C, T, H, W).
        ...
        x_noisy = self.q_sample(x_start=x_start,
                                continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1, 1),
                                noise=noise)
        ...
        if not self.conditional:
            ...
            # print(x_in["INTERPOLATED"].shape, x_in['INTERPOLATED'])
            # print(x_noisy.shape) # B, C, T, H, W. 
            noise_reconstructed = self.denoise_net(torch.cat([x, x_noisy], dim=1),
